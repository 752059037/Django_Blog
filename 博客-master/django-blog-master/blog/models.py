...
class Article(models.Model):
    title = models.CharField('博客标题', max_length=200)  # 博客标题
    category = models.ForeignKey('Category', verbose_name=u'文章类型', on_delete=models.CASCADE)
    date_time = models.DateField('博客日期', auto_now_add=True)  # 博客日期
    content = models.TextField('文章正文', blank=True, null=True)  # 文章正文
    digest = models.TextField('文章摘要', blank=True, null=True)  # 文章摘要
    author = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name=u'作者', on_delete=models.CASCADE)
    view = models.BigIntegerField('阅读数', default=0)  # 阅读数
    comment = models.BigIntegerField('评论数', default=0)  # 评论数
    picture = models.CharField('标题图片地址', max_length=200)  # 标题图片地址
...
class UserInfo(models.Model):
    username = models.CharField('用户', max_length=24)
    password = models.CharField('密码', max_length=24)
